...
def create_user(db: Session, user: schemas.UsersCreate):
    ...
    db_user = models.Users(email=user.email, hashed_password=fake_hashed_password, name=user.name)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
def soft_remove_fundraising(db: Session, fundraising_id: int):
    db.query(models.Fundraisings).filter(models.Fundraisings.id == fundraising_id).update({"deletedAt": func.now()})
    db.commit()
...
def create_organization(db: Session, item: schemas.OrganizationsCreate, user_id: int):
    db_item = models.Organizations(**item.dict(), creatorId=user_id)
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
    return db_item

def update_organization(db: Session, organization_id: int, item: schemas.OrganizationsCreate):
    db.query(models.Organizations).filter(models.Organizations.id == organization_id).update(item.dict())
    db.commit()
    return db.query(models.Organizations).filter(models.Organizations.id == organization_id).first()

def soft_remove_organization(db: Session, organization_id: int):
    db.query(models.Organizations).filter(models.Organizations.id == organization_id).update({"deletedAt": func.now()})
    db.commit()
...
def create_source(db: Session, item: schemas.SourcesCreate, user_id: int):
    db_item = models.Sources(**item.dict(), creatorId=user_id)
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
    return db_item
...
def soft_remove_source(db: Session, source_id: int):
    db.query(models.Sources).filter(models.Sources.id == source_id).update({"deletedAt": func.now()})
    db.commit()
...
def create_post(db: Session, item: schemas.PostsCreate, user_id: int):
    db_item = models.Posts(**item.dict(), creatorId=user_id)
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
    return db_item
...
def soft_remove_post(db: Session, post_id: int):
    db.query(models.Posts).filter(models.Posts.id == post_id).update({"deletedAt": func.now()})
    db.commit()
...
def create_media(db: Session, item: schemas.MediaCreate, user_id: int):
    db_item = models.Media(**item.dict(), creatorId=user_id)
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
    return db_item
...
def soft_remove_media(db: Session, media_id: int):
    db.query(models.Media).filter(models.Media.id == media_id).update({"deletedAt": func.now()})
    db.commit()
...
def create_subscription(db: Session, item: schemas.SubscriptionsCreate, user_id: int):
    db_item = models.Subscriptions(**item.dict(), creatorId=user_id)
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
    return db_item
...
def soft_remove_subscription(db: Session, subscription_id: int):
    db.query(models.Subscriptions).filter(models.Subscriptions.id == subscription_id).update({"deletedAt": func.now()})
    db.commit()